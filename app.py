    ...
    if request.method == "POST":
        ...
        try:
            ...
            keys = [("account:" + hashlib.sha256(username.encode()).hexdigest(), 5),
                    ("source:" + (request.remote_addr or "unknown"), 30),
                    ("global", 300)]
            for key, limit in keys:
                attempt = conn.execute(
                    "SELECT attempts FROM login_attempts WHERE key = ?", (key,)
                ).fetchone()
                if attempt and attempt["attempts"] >= limit:
                    conn.rollback()
                    return Response("Too many login attempts. Try again later.",
...
def transfer():
    ...
    try:
        ...
        abort(400)
    if not amount.is_finite() or amount <= 0 or to_id == u["id"]:
        abort(400)
    ...
    try:
        ...
        if not conn.execute("SELECT id FROM users WHERE id = ?", (to_id,)).fetchone():
            abort(400)
        sender = conn.execute(
            "SELECT id, amount FROM orders WHERE user_id = ? AND status = 'paid' "
            "AND amount >= ? ORDER BY id LIMIT 1", (u["id"], float(amount)),
        ).fetchone()
        if not sender:
            abort(400)
        balance = Decimal(str(sender["amount"]))
        if (not balance.is_finite() or balance > Decimal("999999999999.99") or
                balance != balance.quantize(Decimal("0.01"))):
            abort(400)
        remaining = float(balance - amount)
        debit = conn.execute(
            "UPDATE orders SET amount = ? WHERE id = ? AND amount = ? AND status = 'paid'",
            (remaining, sender["id"], sender["amount"]),
        )
        if debit.rowcount != 1:
            abort(400)
        conn.execute(
...
def go():
    ...
    if (not target.startswith("/") or target.startswith("//") or
            "\\" in target or any(ord(c) < 32 or ord(c) == 127 for c in target)):
        abort(400)
...
def admin_fetch():
    ...
    try:
        ...
        if (parsed.scheme not in ("http", "https") or not parsed.hostname or
                ...
                parsed.port not in (None, 80 if parsed.scheme == "http" else 443)):
            abort(400)
        ...
        # Connect to the validated address, without a second DNS lookup or proxy.
    ...
    try:
        class DataUnpickler(pickle.Unpickler):
            def find_class(self, module, name):
                raise pickle.UnpicklingError("Only plain data is supported")
        raw = bytes.fromhex(blob)
        if any(op.name in ("EXT1", "EXT2", "EXT4") for op, arg, pos in pickletools.genops(raw)):
            raise pickle.UnpicklingError("Extension objects are not supported")
...
def add_headers(resp):
    ...
    if request.endpoint == "static":
        ...
        path = os.path.realpath(os.path.join(app.static_folder, filename))
        if os.path.commonpath([os.path.realpath(UPLOAD_DIR), path]) == os.path.realpath(UPLOAD_DIR):
